...
import { insertDemoDataset } from '../src/services/demoSeedCore'
...
const email =
  process.argv[2] ?? process.env.SEED_DEMO_EMAIL ?? defaultEmail
...
async function main() {
  ...
  await insertDemoDataset(supabase, userId)

  console.log(`OK — démo insérée pour ${email} (${userId}).`)
}
